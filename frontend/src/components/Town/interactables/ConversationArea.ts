...
export default class ConversationArea extends Interactable {
  ...
  private _areaNameOrUndefined?: Phaser.GameObjects.Text;
  ...
  constructor(scene: TownGameScene) {
    ...
    this.townController.addListener('interactableRemoved', (interactableID: string) => {
      if (this.id === interactableID) {
        this._topicText.destroy();
        this._areaNameOrUndefined?.destroy();
      }
    });
  }
  ...
  addedToScene(): void {
    super.addedToScene();
    this._areaNameOrUndefined = this.scene.add.text(
